function [ x_hat, Sp ] = est_srckf_innovate( f_function, x_in, Sp_in, h_function, z_in, Sq_in, Sr_in)  
  ...
  %Sq_in = diag([10^11*ones(1,4) 10^-8 10^-7]);
  %Sq_in = diag([0*ones(1,4) 0 0]);
  %Sq_in = diag([0*ones(1,4) 10^-8 10^-7]);
  [~, R] = qr([1 / sqrt(ckf_m) * ( X_k - Xx ), Sq_in]', 0);
  %[~, R] = qr([1 / sqrt(ckf_m) * ( X_k - Xx )]', 0);
  Sp = R';
